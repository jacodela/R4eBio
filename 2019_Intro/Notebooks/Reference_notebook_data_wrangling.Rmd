...
```{r Load gapminder}
gapminder = read.delim("/Users/jdelacuesta/Nextcloud/Documents/R_for_bio/2019_Data_Wrangling/Data/gapminder.tsv")
gapminder %>% glimpse
```

```{r Load polls}
polls = read.delim("/Users/jdelacuesta/Nextcloud/Documents/R_for_bio/2019_Data_Wrangling/Data/polls_us_election_2016.tsv")
polls %>% glimpse
```
...
You can also create filters combining multiple variables. For example, we could see which countries had a life expentancy below 50 years in Africa in 1966
...
*Excercise:* Would the resulting table be the same if you switched the order of the filters? Why or why not? Try it yourself and compare the resulting tables
...
```{r Select Example 3}
grade_state = polls %>% select(c(pollster, grade, state))
grade_state %>% head
```
...
## Stringr: dealing with text

## Forcats: dealing with categorical variables
