...
ggplot(iris) + geom_point(aes(x=Petal.Width, y=Sepal.Length, col=Species))
```
...
ggplot(iris) + geom_point(aes(x=Petal.Width, y=Sepal.Length, col=Species)) + facet_wrap(facets=c("Species"))
```
...
ggplot(iris) + geom_point(aes(x=Petal.Width,y=Sepal.Length,shape=Species))
# The above with square-root-scaled x-axis
ggplot(iris) + geom_point(aes(x=Petal.Width,y=Sepal.Length,shape=Species)) + scale_x_sqrt()
# The above with both axes scaled by sqrt
ggplot(iris) + geom_point(aes(x=Petal.Width,y=Sepal.Length,shape=Species)) + scale_x_sqrt() + scale_y_sqrt()
# sqrt scale looks almost linear between 4 and 8, the vertical range of this plot
```
...
ggplot(contagion.1969) + geom_point(aes(x=population,y=count,col=disease))
ggplot(contagion.1969) + geom_point(aes(x=population,y=count,col=disease)) + scale_x_log10() + scale_y_continuous(trans="log1p") + facet_wrap("disease",nrow=1)
```
...
ggsave("test2.pdf", plot=iris_plot1)
```
...
ggsave("test3.png",plot=iris_plot1, width=3, height=2, units="in", dpi=300)
```
...
df.example <- data.frame(x=vector1,
                         y=c(vector2,vector3))
ggplot(df.example) + geom_point(aes(x=x,y=y))
```
...
```{r Example line plot}
ggplot(uspolls.abc) + geom_line(aes(x=enddate,y=adjpoll_clinton,group=1))
...
ggplot(uspolls.abc.long) + geom_line(aes(x=enddate,y=adjpoll,group=candidate,col=candidate))
```
...
class(uspolls.abc.long$startdate)
class(uspolls.abc.long$enddate)
# `startdate` and `enddate` parameters have to be converted to class Date
uspolls.abc.long$startdate <- as.Date(uspolls.abc.long$startdate) # Note uppercase D
uspolls.abc.long$enddate <- as.Date(uspolls.abc.long$enddate)
(ggplot(uspolls.abc.long) 
  + geom_line(aes(x=enddate,
                  y=adjpoll,
                  group=candidate,
                  color=candidate))
)
...
ggplot(titanic.long) + geom_bar(aes(x=Class))
```
...
ggplot(titanic.df) + geom_col(aes(x=Class,y=Freq))
```
...
ggplot(titanic.long) + geom_bar(aes(fill=Class,x=1))
```
...
ggplot(titanic.long) + geom_bar(aes(x=Sex,fill=Class))
```
...
```{r Histogram basic}
# Histogram of height
ggplot(hts) + geom_histogram(aes(x=height))
```
...
ggplot(hts) + geom_violin(aes(y=height,x=sex))
```
...
ggplot(hts) + geom_violin(aes(y=height,x=sex,fill=sex))
```
...
ggplot(hts) + geom_boxplot(aes(y=height,x=sex))
```
...
ggplot(hts) + geom_boxplot(aes(y=height,x=sex)) + geom_point(aes(y=height,x=sex))
```
